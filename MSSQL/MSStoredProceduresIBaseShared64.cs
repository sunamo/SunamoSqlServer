...
public partial class MSStoredProceduresIBase : SqlServerHelper
{
    ...
    public int UpdateOneRow(string table, string sloupecKUpdate, object n, ABC abc)
    {
        int pridavatOd = 1;
        string sql = string.Format("UPDATE TOP(1) {0} SET {1}=@p0", table, sloupecKUpdate) + GeneratorMsSql.CombinedWhere(abc, ref pridavatOd);
        SqlCommand comm = new SqlCommand(sql);
        AddCommandParameter(comm, 0, n);
        AddCommandParameteres(comm, 1, abc);
        return ExecuteNonQuery(comm);
    }

    public int UpdateOneRow(string table, string sloupecKUpdate, object n, params AB[] ab)
    {
        return UpdateOneRow(table, sloupecKUpdate, n, new ABC(ab));
    }
    ...
    #region TODO: Removing WebShared
    ...
    public static int AddCommandParameter(SqlCommand comm, int i, object o)
    {
        ...
        else if (o.GetType() == Types.tString || o.GetType() == Types.tChar)
        {
            ...
            //        if (!isNVarChar2)
            //        {
            //            MSStoredProceduresI.ConvertToVarChar(_);
            //        }
            //    }
            //} 
            #endregion
    ...
    public object SelectLastIDFromTableSigned(bool signed, string p, Type idt, string sloupecID, out bool totalLower)
    {
        ...
        //throw new Exception("Nepovolený nehodnotový typ v metodě GetMinValueForType") + 1;
